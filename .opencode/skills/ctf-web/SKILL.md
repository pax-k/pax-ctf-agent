...
## Project ownership and safety

Use this skill as Web challenge methodology inside an authorized engagement.
Under this repository's Mode 1 contract, Strix owns Web execution and its
report. Do not start a direct HexStrike Web lane. Use `hexstrike-web-gap` only
after `INDEX.md` records the exact missing Strix capability. Never target a
system, account, or source that is absent from the written scope.
...
# SQLi quick test
sqlmap --url "https://target.com/page?id=1" --batch --dbs
