...
Use this skill for authorized static review and synthetic-canary validation.
Treat exfiltration, persistence, destructive actions, and log deletion examples
as threat models only. Never use real secrets or affect shared infrastructure.
Follow the project `AGENTS.md` and written engagement scope.
...
Non-executable threat model:

- An attacker can read runner configuration or environment variables and send
  secrets to infrastructure that the attacker controls.
- An attacker can download a persistence payload and register the runner with
  infrastructure that the attacker controls.
- An attacker with a stolen workflow token can enumerate and delete workflow
  runs to remove evidence.

Do not execute these actions. Validate the exposure through static workflow
evidence or a synthetic canary on a disposable runner.
